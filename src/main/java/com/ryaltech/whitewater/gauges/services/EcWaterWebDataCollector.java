...
public class EcWaterWebDataCollector extends AbstractGaugeDataCollector {
	 ...
	 * (java.util.List)
	 */
	public RiverLevel getRiverLevelValues(RiverInfo ri) {
		String gaugeId = ri.getGaugeId();
		...
		try {
			...
			if (httpURLConnection.getResponseCode() != 302) {
				...
				String cookies = extractCookies(httpURLConnection);
				...
				/*
				 * for (String cookie : cookies) { String chunks[] =
				 * cookie.split(";"); if (cookieBuffer.length() > 0)
				 * cookieBuffer.append("; "); cookieBuffer.append(chunks[0]); }
				 */
				...
				if (httpURLConnection.getResponseCode() == 200) {
					...
					if (nodeList != null && nodeList.getLength() == 2) {
						...
						RiverLevel gv = new RiverLevel().setRiverInfo(ri)
					...
					logger.warn(String
							.format("Unable to parse out any gauge values for gauge: %s",
									gaugeId));
					throw new RuntimeException(
							String.format(
									"Unable to parse out any gauge values for gauge: %s",
									gaugeId));

				} else {
					logger.warn(String
							.format("Did not get 200 as HTTP response when trying to retrieve data for gauge: %s",
									gaugeId));
					throw new RuntimeException(
							String.format(
									"Did not get 200 as HTTP response when trying to retrieve data for gauge: %s",
									gaugeId));
				}
			...
			throw new RuntimeException(
					String.format(
							"Unexpected exception when trying to retrieve data from gauge: %s",
							gaugeId), ex);
		}

	}

	private String extractCookies(HttpURLConnection urlConn) {
